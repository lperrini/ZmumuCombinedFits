...
#files=["ztt_mm.output-sm-13TeV-0jet.root","ztt_mm.output-sm-13TeV-1jet_zpt_loose.root","ztt_mm.output-sm-13TeV-1jet_zpt_medium.root","ztt_mm.output-sm-13TeV-1jet_zpt_tight.root","ztt_mm.output-sm-13TeV-2jet_cp.root","ztt_mm.output-sm-13TeV-vbf.root","ztt_mm.output-sm-13TeV-1bjet.root","ztt_mm.output-sm-13TeV-2bjet.root"]
#files=["ztt_mt.output-sm-13TeV-2bjet.root"]
files=["ztt_mt.output-sm-13TeV-0jet.root","ztt_mt.output-sm-13TeV-1jet_zpt_loose.root","ztt_mt.output-sm-13TeV-1jet_zpt_medium.root","ztt_mt.output-sm-13TeV-1jet_zpt_tight.root","ztt_mt.output-sm-13TeV-2jet_cp.root","ztt_mt.output-sm-13TeV-vbf.root","ztt_mt.output-sm-13TeV-1bjet.root","ztt_mt.output-sm-13TeV-2bjet.root","ztt_mt.output-sm-13TeV-MSSM_btag.root","ztt_mm.output-sm-13TeV-0jet.root","ztt_mm.output-sm-13TeV-1jet_zpt_loose.root","ztt_mm.output-sm-13TeV-1jet_zpt_medium.root","ztt_mm.output-sm-13TeV-1jet_zpt_tight.root","ztt_mm.output-sm-13TeV-2jet_cp.root","ztt_mm.output-sm-13TeV-vbf.root","ztt_mm.output-sm-13TeV-1bjet.root","ztt_mm.output-sm-13TeV-2bjet.root","ztt_mm.output-sm-13TeV-MSSM_btag.root"]
labels=["mt-0jet","mt-1jet_zpt_loose","mt-1jet_zpt_medium","mt-1jet_zpt_tight","mt-2jet_cp","mt-vbf","mt-1bjet","mt-2bjet","mt-MSSM_btag","mm-0jet","mm-1jet_zpt_loose","mm-1jet_zpt_medium","mm-1jet_zpt_tight","mm-2jet_cp","mm-vbf","mm-1bjet","mm-2bjet","mm-MSSM_btag"]
filesin=["../../../auxiliaries/shapes/TALLINN/ztt_mt.input-sm-13TeV.root","../../../auxiliaries/shapes/TALLINN/ztt_mm.input-sm-13TeV.root"]
categories=["ch1_prefit","ch2_prefit","ch3_prefit","ch1_postfit","ch2_postfit","ch3_postfit"]
tmp=ROOT.TFile(filesin[1],"r")
h = tmp.Get("mm_1jet_zpt_loose").Get("ZTT")
...
for f in range (0,nfiles):
   file=ROOT.TFile("../outputs/"+files[f],"r")
   for i in range (0,ncat):
      ...
      VBF=file.Get(categories[i]).Get("VBF")
      if(file.Get(categories[i]).GetListOfKeys().Contains("GG")):
         GG=file.Get(categories[i]).Get("GG")
      else:
         GG=h.Clone()
      ...
      if(file.Get(categories[i]).GetListOfKeys().Contains("ZTTsig")):
         ...
         if(file.Get(categories[i]).GetListOfKeys().Contains("ZLL")):
            ...
            ZLL=h.Clone()
      elif(file.Get(categories[i]).GetListOfKeys().Contains("ZTT")):
         ZTT=file.Get(categories[i]).Get("ZTT")
         if(file.Get(categories[i]).GetListOfKeys().Contains("ZLLsig")):
            ZLL=file.Get(categories[i]).Get("ZLLsig")
         elif(file.Get(categories[i]).GetListOfKeys().Contains("ZLL")):
            ZLL=file.Get(categories[i]).Get("ZLL")
         else:
            ZLL=h.Clone()
      else:
      ...
      Data.Draw()
      stack.Draw()
      if( (("mt" in files[f]) and (i==1 or i==4)) or (("mm" in files[f]) and (i==1 or i==2 or i==4 or i==5)) ):  
         xmax = 160
         xmin = 30
      ...
      c2.SaveAs("mvis_"+labels[f]+"_"+categories[i]+".pdf")
